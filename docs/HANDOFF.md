...
## Planning SIRH connecté

Le portail affiche les fiches de mission proposées à l’équipier depuis la passerelle SIRH. L’équipier peut accepter une mission ou demander sa correction avec un motif. La configuration du second fournisseur est écrite par ARCenal Bridge dans `config.php` sous les clés `sirh_*`; elle ne doit jamais être saisie ou modifiée dans le répertoire web.
